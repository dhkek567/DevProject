...
public class AOPController {
	 ...
	 * 		반장 태원학생은 부탁받은 요구사항을 이행하기 위해서 본인이 만들고 있는 서비스 로직에서 처리 속도를 찍어볼 메소드를 개발해 처리 속도가 잘 찍히는걸
	 * 		확인합니다. 반장 태원이는 아주 흡족하고 기분이 좋았습니다.
	 * 		기분이 좋은 태워학생은 팀장님께로 달려갑니다.
	 * 		그리고 재원 팀장님께서 해당 내용을 컨펌 받았습니다.
	 * 		재원팀장님은 아주 긍정적인 검토안을 태원학생한테 전달하면서 우리 서비스 전체에도 각 처리속도를 찍어제수요 라고 다시 부탁합니다.
	 * 		태원학생은 본인이 만들어 낸 메소들를 각 기능별 서비스 로직에 하나하나씩 약 20만개쯤 작성하던 중, 의문을 갖습니다...
	 * 
	 * 		" 서비스 로직에서 제일 중요한 로직은 본래의 기능이 제일 중요하고 지금 내가 작성하는 로직은 옵션(부가 기능) 이 추가되는게 아닌가?
	 * 		 그럼 이걸 하나의 묶음으로 처리가 불가능한건가?"
	 * 
	 * 		하는 생각을 하게 됩니다.
	 * 		여기서, 시간을 측정하고 권한을 체크하는 등의 기능은 옵션과 같은 부가 기능으로 일종의 인프라 로직이라고 하는데,
	 * 		이 인프라 로직은 애플리케이션 전 영역에서 나타날 수 있고, 중복코드를 만들어 내 개발의 효율성을 저하시키고 유지 보수를 힘들게 한다.
	 * 	
	 * 		이러한 인프라 로직은 아래처럼 하나의 관심사를 가질 수 있는 이런 관심사들의 중복이 횡단으로 나타나는데,
	 * 		이것을 가르켜 '횡단관심사(Cross-Cutting Concern)'라고 한다.
	 * 
	 * 		┌────────────────────────────────────────────────────────────┐
	 * 		│		[처리속도 측정]  [처리속도 측정]  [처리속도 측정]  [처리속도 측정] 	 │ 	
	 *		└────────────────────────────────────────────────────────────┘ 
	 *				[비즈니스로직]	 [비즈니스로직]	   [비즈니스로직]	[비즈니스로직]
	 *				[처래 내용로직]	 [처래 내용로직]   [처래 내용로직]  [처래 내용로직]
	 *		└────────────────────────────────────────────────────────────┘
	 ...
	 *			여러 모듈에서 공통적이고 반복적인 처리를 요구하는 내용(메소드 실행시작 시간 출력, 메소드 처리 후 시간 출력 등)
	 ...
	 *				- Around : 조인 포인트 전 후에 실행(삼겹살을 굽기 직전과 먹은 직후 실행) (!제일 많이 사용!)
	 *
	 *
	 *
	 *		# AOP(관점 지향 프로그래밍(Aspect Oriented Programming))
	 *		- 관점 지향 프로그래민(Aspect Oriented Programming)을 의미하는 약자이다.
	 *			
	 *			1-1) 관점 지향 프로그래밍
	 *			
	 ...
	 *				> 핵심 비즈니스 로직과 다고 거리가 있지만, 여러 모듈에서 공통적이고 반복적인 처리를 요구하는 내용이다.
	 *		
	 *				- 횡단 관심사 분리(Seperation of Cross_Cutting Concern)
	 *				> 애플리케이션을 개발할 때 횡단 관심사에서 해당하는 부분을 분리해서 한 곳으로 모으는 것을 의미한다.
	 *
	 *			1-2) AOP 개발 순서
	 *			
	 ...
	 *			> 어떤 부가기능을 구현할 것인지 결정(Before, After, AfterRetunning, AfterThorwing, Around)
	 *
	 *			- 포인트컷(PointCut)
	 *			> 수 많은 조인포인트 중에서 실제로 어드바이스를 적용할 곳을 선별하기 위한 표현식(Expression) 을 말한다.
	 *			> 어드바이스가 적용 될 지점
	 *
	 *			- 위빙(Weaving)
	 ...
	 *
	 *		- 의존관계 등록(pom.xml 설정)
	 *			> asectjrt (pom.xml 안에 이미 설정되어 있음)
	 ...
	 *			public void startLong(JoinPoint jp){
	 ...
	 *  ** crud board에서 delete기능을 에러로 실행한다(쿼리 no = 2, no 2 = 로 변경)
	 */
	...
	/*
	 * 8. 메소드 정보 획득
	 * - @Before 어노테이션이 붙은 메소드는 JoinPoint라는 매개 변수를 통해 실행중인  메소드의 정보를 구할 수 있다.
	 */
