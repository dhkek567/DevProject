...
public class AjaxMemberController {
	...
	// 1) URL 경로상의 여러개의 경로 변수값을 @PathVariable 어노테이션을 지정하여 여러개의 문자열 매개변수로 처리한다.
	...
	//4)객체 타입의 JSON 요청 데이터는@PathVariable 어노테이션을 지정한 문자열 매개변수와 @RequestBody 어노테이션을 지정한 자바빈즈 매개변수로 처리한다 .
	...
	public ResponseEntity<String> register07(@RequestBody Member member){
		...
		if(cardList !=null && cardList.size() > 0) {
			...
			for(int i=0; i < cardList.size(); i++) {
				...
				log.info("getVaildMonth() : "+card.getValidMonth());
			}
		}else {
