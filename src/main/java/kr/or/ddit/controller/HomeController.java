...
public class HomeController {
	...
	// 반환값이 sub/home02003 이므로 뷰(/sub/home0203.jsp)를 가리킨다.
	...
	// DevProject\WEB-INF\views\sub\home0205.jsp와 같은 경로로 설정
	 ...
	 * 
	 * @ResponseBody를 지정하지 않으면 HTTP 404 에러가 발생한다.(jackson-databind 라이브러리가 설정되어 있지 않는 경우)
	 ...
	 */
	// 200 OK 상태 코드를 전송한다.
	// Void 타입은 아무런 형태가 아닌데 제너릭타입의 무언가는 채워야겠어서 채우는 Placeholder와 같은 느낌이랄까?
	@ResponseBody
	...
	public ResponseEntity<byte[]> goHome1101(){
		...
		try {
			...
			in = new FileInputStream("E:/99.JSP&SPRING/02.SPRING2/image/유나.jpg");
			// 이미지를 내보낼 때는 미디어 타입 설정 필수 
	...
	public ResponseEntity<byte[]> goHome1102(){
		...
		String fileName = "유나.jpg";
		InputStream in = null;
		ResponseEntity<byte[]> entity = null;
		
		try {
			HttpHeaders header = new HttpHeaders();
			in = new FileInputStream("E:/99.JSP&SPRING/02.SPRING2/image/"+fileName);
			//(APPLICATION_OCTET_STREAM) 내가 내보낼 데이터를 이진 데이터로 하겠다
			header.setContentType(MediaType.APPLICATION_OCTET_STREAM);
			// 아래의 값들을 가지고 넘어가야 다운로드가 진행이 된다.
			header.add("Content-Disposition", "attachment; filename=\""+new String(fileName.getBytes("UTF-8"), "ISO-8859-1")+
					"\"");
			// HttpStatus.CREATED 로 설정해야 다운로드가 가능하다
