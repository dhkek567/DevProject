...
public class JSTLHomeController {
	...
	/*
	 * 5. 표준 태그 라이브러리(JSTL)
	 * - 많은 개발자들이 JSP에서 코드를 깔끔하게 작성하기 위해서 커스텀 태그를 만들어 왔는데, 이런 중복되는 노력을 없애기 위해서 나온것이 바로 JSTL이다. 
	 * 
	 *  	1) core 태그 라이브러리
	 *  	
	 *  	요소			| 		설명
	 ...
	 * ───────────────────────────────────────────────────────────────────────────────────────────────
	 * 
	 * 		#taglib 지시자 사용 모습  ::: <%@ tablib uri="http"//java.sun.com/jsp/jstl/core" perfix="c" %>
	 * 			>인터페이스에 해당하는 core uri를 잘 확인하고 넣어야한다.
	 * 
	 * 2) fmt 태그 라이브러리
	 * 
	 *   	요소					| 		설명
	 ...
	 *  	fmt:parsetDate		| 문자열을 Date 객체로 변환한다.
	 *──────────────────────────────────────────────────────
	 *
	 *		#taglib 지시자 사용 모습  ::: <%@ tablib uri="http"//java.sun.com/jsp/jstl/fmt" perfix="fmt" %>
	 * 			>인터페이스에 해당하는 fmt uri를 잘 확인하고 넣어야한다.
	 * 
	 * 3) function 태그 라이브러리
	 * 
	 *   	요소							| 		설명
	 ...
	 *  	fn:split					| 문자열 구분자로 분할해서 문자열 배열을 만든다.
	 * ─────────────────────────────────────────────────────────────────────────────────────────────────
	 *  
	 *
	 *		#taglib 지시자 사용 모습  ::: <%@ tablib uri="http"//java.sun.com/jsp/jstl/function" perfix="function" %>
	 *			> perfix에 설정한 내용은 자동 생성이긴하지만, 내가 원하는perfix명이 있다면 작성 가능(기본 fn, custom function으로 설정가능)
	 * 
	 * 
	 * 
	 * 
	 * 6. 코어 태그 
	 * - 조건분기나 반복처리 그리고 변수의 지정등과 같이 논리적인 처리를 위해 스크립트 코드를 대처하기 위한 태그를 제공한다. 
	 * 
	 * 	1) <c:out>
	 ...
	 *────────────────────────────────────────────────────────────────────
	 * 		test		|	boolean	| 검사 조건
	 * 		var			|	String	| 검사 조건의 계산 결과를 저장 할 EL 변수
	 * 		scope		|	String	| 삭제할 변수가 포함된 영역
	 *────────────────────────────────────────────────────────────────────
	 *
	 *  6) <c:choose> 
	 *  - 여러 조건을 처리할 때 사용한다.
	 *  - c:when, c:otherwise 가 사용된다.
	 *  
	 *  7) <c:when>
	 *  - 여러 조건을 지정하고 지정한 조건과 일치하는 처리 내용을 구현한다. <c:choose> 요소에서 사용한다.
	 *  	속성 	 		|	타입		| 		설명 
	 *────────────────────────────────────────────────────────────────────
	 *   	test		|	bolean	| 본문 내용을 처리할지 여부를 결정하는 테스트 조건
	 *────────────────────────────────────────────────────────────────────
	 *  8) <c:otherwise>
	 *  - <c:when> 요소에서 지정한 조건이 모두 일치하지 않을 때 처리할 내용을 구현한다. <c:choose> 요소에서 사용한다.
	 *  
	 *  
	 *  9 <c:forEach>
	 ...
	 *	- 상대 URL : 
	 *		> 웹 애플리케이션 내에서의 절대 경로 : '/'(슬러시)로 시작한다. ex) /board/list.jsp
	 *		> 현재 JSP에 대한 상대 경로 : '/'(슬러시)로 시작하지 않는다. ex) .../board/list.jsp
	 *
	 *	12) <c:url>
	 *	- URL을 재작성한다.
	 *		속성 	 		|	타입			| 		설명 
...
//		member.setUserId("<p>hongkd<>&$0102</p>");
	...
	// 3) memeber 객체의 userId 프로퍼티 값을 활용하여 값을 설정한다.
	...
	// c:forTokens
	// delims 속성이 지정된 구분자를 사용하여 items 속성이 전달된 문자열을 나누고 var 속성에 명시한 변수에 나뉘어진 문자열을 지정한다.
