...
public class MemberController {
	...
	public String registerByPath(@PathVariable String userId) {
		...
		// 경로상에 포함되어 있는 값은 @PathVariable을 꼭 붙여주어야 값을 바인딩 할 수 있다.
	...
	// 4)HTML Form 필드가 여러개일 경우에 컨트롤러 매개변수의 위치는 상관없다.
	...
	//3) @RequestParam 어노테이션을 사용하여 특정한 HTML Form 필드명을 지정하여 요청을 처리한다.
	 ...
	 * 4. 요청 처리 자바빈즈
	 */
	//1) 폼 텍스트 필드 소요값을 자바빈즈 매개변수로 처리한다.
	...
	public String registerJavaBeans02(Member member, int coin, String userId) {
		...
		// name 값만 같으면 뭐든 상관없이 다 자동 바인딩 된다
		// 그래서 coin이 두 개가 나옴 
	...
	/*
	 * 5. Date 타입 처리
	 * - 스프인 MVC Date 타입의 데이터를 처리하는 여러 방법을 제공한다.
	 ...
	 * -@DateTimeFormat 어노테이션 pattern 속성값에 원하는 날짜 형식을 지정할 수 있다.
	 * 
	 * 테스트는  /registerByGet02를 요청하고 파라미터로 받아 줄, Member 클래스의 날짜를 받는 멤버 변수에 어노테이션을 설정합니다. 
	 */
	...
	//5) 폼 selectBox 요소값을 기본 데이터 타입인 문자열 타입 매개변수로 처리한다.
	...
	//10) 폼 체크박스 요소 값을 기본 데이터 타입인 배열 타입 매개변수로 처리한다.
	...
	public String registerCheckbox03(ArrayList<String> hobbyList) {
		// 받는 타입을 List하게 되면 [No primary or default construct found for interface java.util.List]에러 발생
		...
		if(hobbyList != null && hobbyList.size()>0) {
			log.info("hobbyArray.size() : "+hobbyList.size());
	...
	//14) 폼 택스트 필드 요소값 자바빈즈 매개변수로 처리한다.
	...
	public String registerAddress(Address address) {
		...
		if(address != null) {
			...
			log.info("location.getLocation : "+ address.getLocation());
		}else {
	...
	// 16) 폼 택스트 핑드 요소값을 중첩된 자바빈즈 매개변수로 처리한다.
	...
	//18) 폼 택스트 영역 요소값을 Date 타입 매개 변수로 처리한다.
	...
	//19) 폼 택스트 영역 요소값을 @DateTimeFormat 어노테이션을 지정하여 Date 타입 매개 변수로 처리한다.
	...
	public String registerUser(Member member,MultipartFile picture,Model model) {
		...
		log.info("picture.getOriginalFilename() >>>>>>>>>>>>>>> "+picture.getOriginalFilename());
		log.info("picture.getContentType() >>>>>>>>>>>>>>> "+picture.getContentType());
		...
		log.info("birth : "+member.getBirthDay());
		log.info("gender : "+member.getGender());
		
		if(member.getGender().equals("male")) member.setGender("남자");
		if(member.getGender().equals("female"))member.setGender("여자");
		if(member.getGender().equals("other"))member.setGender("성별 미선택");
		...
		log.info("member.isFreigner() : "+member.isForeigner());

		log.info("nationality : "+member.getNationality());
		if(member.getNationality().equals("Korea")) {member.setNationality("대한민국");}
		if(member.getNationality().equals("Germany")) {member.setNationality("독일");}
		if(member.getNationality().equals("Austrailia")) {member.setNationality("호주");}
		if(member.getNationality().equals("Canada")) {member.setNationality("캐나다");}
		log.info("carArray : "+member.getCarArray());
		
		String[] carArray = member.getCarArray();
		String cars = "";
		if(carArray !=null && carArray.length>0) {
			for(String car : carArray) {
				if(car.equals("volvo"))cars += car.toUpperCase()+" ";
				if(car.equals("jeep"))cars += car.toUpperCase()+" ";
				if(car.equals("bmw"))cars += car.toUpperCase()+" ";
				if(car.equals("audi"))cars += car.toUpperCase()+" ";
		...
		log.info("hobbyArray : "+member.getHobbyArray());
		
		String[] hobbyArray = member.getHobbyArray();
		String hobby = "";
		if(hobbyArray != null && hobbyArray.length>0) {
			for(String hobbys : hobbyArray) {
				if(hobbys.equals("Sports")) hobby +="운동 ";
				if(hobbys.equals("Music")) hobby +="음악감상 ";
				if(hobbys.equals("Movie")) hobby +="영화 ";
			}
		}else {
			hobby = "취미없음";
		}
	...
	//2) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을  MultipartFile 매개 변수와 문자열 매개변수로 처리한다.
	...
	//3) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을  MultipartFile 매개 변수와 자바빈즈 매개변수로 처리한다.
	...
	//5) 5) 여러개의 파일업로드 폼 파일 요소값을  MultipartFile 매개 변수로 처리한다.
	...
	public String registerFile05(Member member, MultipartFile picture,MultipartFile picture2) {
		...
		log.info("originalNam : "+ picture.getOriginalFilename());
		log.info("size : "+ picture.getSize());
		log.info("contentType : "+ picture.getContentType());
		
		
		log.info("originalNam2e : "+ picture2.getOriginalFilename());
	...
	public String registerFile06(List<MultipartFile> pictureList) {
		...
		for(int i =0; i<pictureList.size(); i++) {
			...
			log.info("originalNam ["+i+"]: "+ picture.getOriginalFilename());
	...
	//7) 여러개의 파일업로드를 폼 파일 요소값을 MultipartFile 타입의 요소를 가진 리스트 컬렉션 타입 매개변수로 처리한다. 
