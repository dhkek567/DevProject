...
public class DatabaseConnectController {
	 ...
	 * - Oracle win32_lgR2_client와 OracleXE112_Win64로 오라클 클라이언, 서버를 설치
	 * 
	 * 		# Oracle Client 및 Server 설치 참고
	 * 		- https://junesker.tistory.com/79 (오라클 클라이언트 설치)
	 * 		- https://junesker.tistory.com/80 (오라클 클라이언트 설치)
	 *
	 * 2. Oracle SQLDveloper 설치
	 * 	- sqldeveloper를 설치
	 ...
	 *  			constraint pk_board increment by 1 start with 1 nocahe;
	 *  
	 *  		);
	 *  
	 *  	3) 작성할 화면(기본화면으로 진행)
	 ...
	 *  	- update
	 *  	: 데이터를 변경하는 SQL을 실행 할 때 사용한다.
	 *  
	 *   	# 스프링 JDBC 설정
	 *   	
	 *   	- spring-jdbc 의존관계 설정
	 *   		> spring-jdbc 설정 처리(pom.xml)
	 *   	- 데이터 소스 설정
	 ...
	 *  	# 게시판 구현 설명
	 *  		
	 *  		기본적인 CURD를 바탕으로 게시판 구현입니다.
	 *  		등록 , 수정, 삭제, 상세보기 등의 페이지를 구현하여 JDBC 방식 기반의 CURD를 작성합니다.
	 ...
	 *  		설정된 Entity들을 이용하여 jsp 객체를 통해서 함수를 호출하면(persist, find 등등) 호출된 함수를 통해 Spring Data JPA가
	 *  		자동으로 Entity를 분석하여 함수에 따른 쿼릴르 자동생성하여 결과를 실행, 조회, 수정 등을 처리해준다.
	 *  
	 *  		JPA는 JPA 구현체인 hibernate를 이용하여 사용합니다.(입력값 검증을 위해서 추가한 hibernate 그거임)
	 *  		그러기 위해선 관련 의존관계를 등록 후 사용해야합니다. (관련 라이브러리는 hibernate 말고도 설정에 따라 다양할 수 있음)
	 ...
	 *  
	 *  		** 데이터베이스 설계 라인이 정확하게 설계되어 있는 상태에서는 JPA가 가독성이 좋다지만, 우리는 Mybatis를 활용해 CRUD를 진행!
	 *  
	 *  		[용어]
	 *  
	 *  			** ORM이란?
	 *  				- 객체에 데이터를 읽고 쓰는 방법으로 관계형 데이터에비스에 데이터를 읽고 쓰는 기술입니다.
	 *  
	 *  			** 기업 면접에서 ORM은 뭐 쓰셨어요? 라는 질문을 받는 경우가 있다. (경력직 또는 신입 면접에서)
	 *  				- 저는 ibatis를 사용했습니다.
