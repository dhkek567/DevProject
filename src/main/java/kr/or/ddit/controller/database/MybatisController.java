...
public class MybatisController {
	 ...
	 * 			마미바티스는 Apache 라이선스 2.0으로 배포되는 자유 소프트웨어다. 
	 * 		
	 ...
	 * 		> basePackage
	 * 
	 * 		- 마이바티스 설정 ( mybatisAlias.xml)
	 * 			> 마이바티스 위치 설정은 root-context.xml 의 sqlSessinFactory 설정할 때 property 요소로 적용
	 * 
	 * 		# 테이블 생성
	 * 
	 ...
	 *				> 게시판 등록 화면 컨트롤러 메소드 만드릭(crudRegister:get)
	 ...
	 *				> 게시판 수정 화면 인터페이스 메소드 만들기
	 *				> 게시판 수정 화면 클래스 메소드 만들기
	 ...
	 *				> 게시판 수정 기능 인터페이스 메소드 만들기
	 *				> 게시판 수정 기능 클래스 메소드 만들기
	 ...
	 *			> 기존에 kr.or.ddit.vo.Board와 같은 패키지명 + 클래스명으로 되어 있는 type설정을
	 *				alias로 설정된 별칭을 사용한다.
	 *
	 *
	 *4. '_'로 구분된 컬럼명 자동 매핑
	 *-	 마이바티스 설정의 mapUnderscoreCamelCase 프로퍼티 값을 true로 지정하면 '_'로 구분된 컬럼명을
	 *	소문자 낙타표기법의 프로퍼티명으로 자동 매핑할 수 있다.
	 *
	 *		'_' 포함되어 있는 데이터베이스 컬럼명을 카멜기법 셋팅으로 인해서 bo_no 가 boNo로 처리된다.
	 *
	 *		# 마이바티스 설정(mybatisAlias.xml 설정)
	 *	
	 *
	 *		<settings>
	 *			<setting name="mapUnderscoreCamelCase" value="true"/> 설정 추가
	 *		</settings>
	 *
	 *		매핑 파일 수정(boardMapper_SQL.xml)
	 *		- read, list 로 되어있는 실행 xml에서 select 절 내에서 조회되고 있는 '_'가 포함되어 있는
	 *			컬럼들은 예로 as boardNo or as regDate와 같이 설정되어 있다면 삭제
	 *			카멜케이스에 의해서 '_'로 되어 있는 컬럼은 자동 변환되므로 
	 *
	 *
	 *5. 기본키 취득
	 *-	마이바티스 useGeneratedKeys 속성을 이용하여 insert 할 때 데이터베이스 측에서 채번된 기본키를 취득 할 수 있다.
	 *
	 *		# 마이바티스 설정
	 *	
	 *			- 매핑 파일 수정(boardMapper_SQL.xml)
	 ...
	 *					)values (
	 *						#{baordNo}, #{title}, #{content}, #{writer}, sysdate
	 *					)
	 *				</insert>
	 *
	 ...
	 *
	 *7. 일대다 관계 테이블 매핑
	 *- 마이바티스 기능을 활용하여 매핑 파일을 적절하게 일대다 관계 테이블 매핑을 쉽게 처리할 수 있다. 
	 *
	 *		# 게시판 구현 설명
	 *		
	 ...
	 *
	 *
	 *			- 회원 목록 화면 컨트롤러 메소드 만들기 (crudMemberListr:get)
	 ...
	 *
	 *		┌──────────────────────────────────────────────────────┐
	 *      │   페이지 모듈화를 위한 Tiles를 함께 사용하여 CRUD를 진행합시다.	   │
	 *      └──────────────────────────────────────────────────────┘
	 *       
	 *       1. Tiles란?
	 *       
	 *       	- 어떤 jsp를 템플릿으로 사용하고 템플릿의 각 영역을 어떤 내용으로 채울지에 대한 정보를 설정한다.
	 *       	- 하나의 화면들을 만들어본다면 공통적이로 반복적으로 생성해야 하는 header, footer와 같은 영역들이 존재합니다.
	 *       		우리는 그러한 공통 부분들을 분리하여 반복적으로 컴포넌트들을 사용하는게 아닌 공통적인 부분은 한번만 가져다 쓰고
	 *       		변화하는 부분에 대해서만 동적으로 변환해 페이지를 관리 할 수 있어야 할것입니다.
