...
public class ExceptionController {
	 ...
	 * 		이런 점을 고려하면 최대한 사용자가 직접 예외를 처리하여 사용자가 정의한 에러 페이지를 표시하게 해야한다.
	 * 
	 * 		1-1) 예외 종류
	 * 		- 스프링 프레임워크 예외
	 * 		- 사용자 정의 예외
	 ...
	 * 
	 * 		1-3) 예외처리 불가 기준
	 * 		- 사용자가 정의한 예외 처리를 거치지 않은 예외로 프레임워크에서 이러한 예외를 처리한 기본적인 내용은 아래와 같다.
	 ...
	 * 				> Http Status 400
	 * 				> [예외 상황 만들 위치]
	 *					kr.or.ddit.controller.board.CurdBoardController curdRegister()
	 ...
	 *				> Http Status 500
	 *				> [예외 상황 만들 위치]
	 *					view/crud/register.jsp title 입력 input의 id, name을 title2로 수정
	 *
	 *
	 ...
	 *				>[요청 URL]
	 *					/crud/board/delete?boardNo=43 (psot)
	 *
	 ...
	 *  		> Contoroller 에서 선언한 URL 매핑 정보로도 설정 가능
	 *  		> 서블릿 3.1 이상 지원
	 *  
	 * 		- 처리할 수 있는 예외	
	 * 			- A. 등록할 때 제목에 빈 값을 입력하여 유효성 검증 예외 발생
	 ...
	 * 
	 * 	6. 예외 처리 어노테이션 
	 * 	- @ExceptionHandle @ControllerAdvice를 이용하여 처리한다.
	 * 
	 * 		예외 처리 방법
	 ...
	 *  		> 입력값 검증 대상인 자바빈즈 메소드 매개변수에 @Validated를 지정하고 바로 다음에 BindingResult 를 정의한다.
