...
public class FileUploadController01 {
	 ...
	 *
	 *				- StnadardServletMultipartResolver 사용 시, 설정
	 *					> servlet 3.0의 part를 이요한 MultipartFile 데이터 처리,
	 ...
	 *					> bean 속성으로 maxUplaodSize, maxInMemorySize 및 defaultEncoding 설정을 제공합니다.
	 *					> 기본값 및 허용되는 값에 대한 자세한 내용은 각 DiskFileUpload 속성을 참조
	 *					> pom.xml 설정
	 *						> commons-fileuplaod 추가
	...
	*			- 파일 업로드 목록 서비스 인터페이스 메소드 만들기
	*			- 파일 업로드 목록 서비스 클래스 메소드 만들기
	*			- 파일 업로드 목록 화면 Mapper 인터페이스 메소드 만들기
	*			- 파일 업로드 목록 화면 Mapper xml 쿼리 만들기
	*			-		여기까지 확인
	*
	*
	*			- 파일 업로두 수정 화면 컨트롤러 메소드 만들기 (itemModifyForm:get)
	...
	*/
	...
	public String itemRegister(Item item, Model model) throws IOException {
		...
		item.setPictureUrl(createdFileName); //아래 메소드에서 반환된 createdFileName
	...
	public String itemRemove(Item item, Model model) {
		...
		log.info("item.getItemId()>>>>>>>>>>>>>>>>>>>> "+ item.getItemId());
	...
	// 확장자에 맞는 미디어타입(ContentType) 가져오기
	...
	private String uploadFile(String originalName, byte[] fileData ) throws IOException {
		log.info(resourcePath); // root-contetxt.xml에서 설정한 uploadPath
