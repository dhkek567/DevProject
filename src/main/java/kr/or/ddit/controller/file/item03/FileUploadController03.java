...
public class FileUploadController03 {
	 ...
	 * 
	 * 
	 * 			- 파일 업로드 수정기능 컨트롤러 메소드 만들기 (item3Modify:post)
	 * 			- 파일 업로드 수정기능 서비스 인터페이스 메소드 만들기
	 * 			- 파일 업로드 수정기능 서비스 클래스 메소드 만들기
	 * 			- 파일 업로드 수정기능 Mapper 인터페이스 메소드 만들기
	 * 			- 파일 업로드 수정기능  Mapper xml 쿼리 만들기
	 * 			-		여기까지 확인
	 * 
	 * 
	 * 			- 파일 업로드 삭제화면 컨트롤러 메소드 만들기 (item3RemoveForm:get)
	 * 			- 파일 업로드 삭제화면 서비스 인터페이스 메소드 만들기
	 * 			- 파일 업로드 삭제화면 서비스 클래스 메소드 만들기
	 * 			- 파일 업로드 삭제화면 Mapper 인터페이스 메소드 만들기
	 * 			- 파일 업로드 삭제화면  Mapper xml 쿼리 만들기
	 * 			- 파일 업로드 삭제화면  만들기(item3/remove.jsp)
	 * 			-		여기까지 확인
	 * 
	 * 
	 * 
	 * 			- 파일 업로드 삭제기능 컨트롤러 메소드 만들기 (item3Remove:post)
	 * 			- 파일 업로드 삭제기능 서비스 인터페이스 메소드 만들기
	 * 			- 파일 업로드 삭제기능 서비스 클래스 메소드 만들기
	 * 			- 파일 업로드 삭제기능 Mapper 인터페이스 메소드 만들기
	 * 			- 파일 업로드 삭제기능  Mapper xml 쿼리 만들기
	 * 			-		여기까지 확인
	 * 
	 */
	...
	public String item3Register(Item3 item, Model model) {
		// 여러개의 파일을 받기 위해서는 Item3 객체를 생성하여 거기에 받는다 
		// 
	...
	public String itme3Modify(Item3 item, Model model) {
		String files[] = item.getFiles();
		for(int i=0; i<files.length; i++) {
			log.info("files["+i+"] : "+files[i]);
		}
	...
	public ResponseEntity<String> uploadAjax(MultipartFile file) throws IOException{
		...
		// 아래 나와있는 결과를 얻어온다 
		// saveName은 / 2023/09/04/UUID_원본 파일명을 리턴한다.
