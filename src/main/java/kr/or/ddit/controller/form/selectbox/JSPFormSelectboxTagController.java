...
public class JSPFormSelectboxTagController {
	...
	//3) 모델에 CodeLabelValue 타입의 요소를 가지는 List 데이터를 생성하여 추가한 후에 화면에 전달한다.
	...
	public String registerFormSelectboxResult2(Model model, Member member) {
		...
		if(carList != null && carList.size()>0) {
			...
			for(int i = 0; i < carList.size(); i++) {
				log.info("carLsit("+i+") : "+carList.get(i));
			}
		}else {
