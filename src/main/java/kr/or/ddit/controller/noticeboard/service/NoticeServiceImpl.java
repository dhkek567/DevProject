...
public class NoticeServiceImpl implements INoticeService{
	...
	public ServiceResult insertNotice(HttpServletRequest req, NoticeVO noticeVO) {
		...
		if(status > 0) {
			...
			try {
				// 공지사항 파일 업로드 처리 함수
				// 공통적인 소스가 반복되므로 함수로 모듈화 진행하여 사용한다.
...
//			try {
//				tst.sendGet("이종보통 보통이 아님", noticeVO.getBoTitle());
//			} catch (IOException e) {
//				e.printStackTrace();
//			}
			result = ServiceResult.OK;
			...
			try {
				...
				if(delNoticeNo != null) {
					for(int i=0; i < delNoticeNo.length; i++) {
						NoticeFileVO noticeFileVO =	noticeMapper.selecNoticeFile(delNoticeNo[i]);
						noticeMapper.deleNoticeFile(delNoticeNo[i]); // 파일번호에 해당하는 파일 데이터를 삭제 
		...
		if(status > 0) {
			...
			try {
				if(noticeFileList != null && noticeFileList.size()>0) {
					...
					// [1] = a2e2ygass-a2d4d3f-a23faa4-fafafadf_원본파일명.jpg
		...
		try {
			if(folder.exists()) { // 경로가 존재한다면
				...
				for(int i=0; i < folderList.length; i++) {
					if(folderList[i].isFile()) { // 폴더안에 있는 파일이 파일일때
