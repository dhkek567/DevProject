...
public class ImageUpload {
	...
	public String imageUpload(
		...
		// CKEditor4 본문 내용에 이미지를 업로드 해보면 'upload'라는 키로 파일 데이터가 전달되는 것을 확인할 수 있다.
		...
		if(file != null && file.getSize() > 0 && StringUtils.isNotBlank(file.getName())) {
			...
			if(file.getContentType().toLowerCase().startsWith("image/")) {
				...
				if(file.getSize() > limitSize) { // 최대 크기 2MB보다 클 때
					 ...
					 * 		"error" : [
					 * 					{
					 * 				"message" : "2MB 미만의 이미지 업로드만 가능합니다."
					 * 					}
					 *				 ]
					 * }
					 */
					...
					jsonMsg.addProperty("message", "2MB 미만의 이미지 업로드만 가능합니다.");
					jsonArr.add(jsonMsg);
					...
					try {
						...
						//업로드 경로로 설정된 폴더구조가 존재하니 않는 경우, 파일을 복사할 수 없으므로 폴더 구조가 존재하지 않는 경우 생성하고
						// 존재하는 경우 건너뛰도록 한다.
						 ...
						 * 
						 * 		"uploaded" : 1,
						 * 		"fileName" : "xxxxxxxxxx-xxxxxxxxx.jpg",
						 * 		"url" : "/resources/img/xxxxxxxxxx-xxxxxxxxx.jpg",
						 * }
						 */
