...
public class TransactionController {
	 ...
	 * 3. 트랜잭션 적용
	 * 
	 * 		[테스트는 AOP가 적용된 CurdBoardController와 연계된 ServiceImpl에서 진행]
	 * 
	 *  	트랜잭션을 사용하는 예 상황이 존재한다.
	 *  	예) 회원은 반드시 하나의 권한을 가진다는 비즈니스 규칙이 있습니다.
	 *  		회원과 회원권한 테이블이 각각 개별로 존재하지만 회원 정보를 저장할 때 반드시 회원 권한 정보를 동시에 저장해야 합니다.
	 *  		클래스나 메소드에 대해 @Tracsactional 어노테이션을 부여하여 트랜잭션을 적용 할 수 있습니다.
	 *  
	 *  		서비스 구현 객체에 트랜잭션 적용
	 ...
	 *  	
	 *  	"블로그에 분명 적용하면 된다고 헀는데 왜 안되지?"
	 *  	"내가 설정을 잘못했나? 설정이 어디부터 잘못된거지?"
	 *  	"디버깅을 일단 해보자 데이터가 뭔가 문제가 있나보다,,,"
	 *  
	 ...
	 *  	기본적으로 스프링에서 트랜잭션 처리는 RuntimeException계열과 Error에 대해서만 Rollback 처리를 한다.
	 *  	
	 *  
	 *  	여기서, 트랜잭션으로 국한된 롤백 정책이 아니라 스프링 프레임워크에서의 기본 정책에 대한 내용일 뿐이다.
	 *  	실제로 '트랜잭션 롤백 처리', 'checkedException vs uncheckedException' 등 검색해보면 잘목된 정보들로 복사/붙여넣기 되어 있는
	 *  	내용이 수두룩하여 많은 에러 정보를 공유하고 있다. (모든 블로그가 그렇진 않지만, 대표적인 표가 그렇게 많이 공유됨....;;)
	 *  
	 *  	**
	 *  	'예외처리 시 트랜잭션 처리' 에 대한 정보가 잘못되어 표기된 경우가 많다.
	 *  	이럴 때 되고 저럴 때 되고가 아니라, 기본적인 스프링 트랜잭션의 정책이 있고 그 안에서 정책 이 외에 옵션으로 특정 에러가 발생했을 때
	 *  	롤백을 진행할 수 있도록 제공한다. ( 많은 블로그들을 100% 믿지말길 바랍니다.... 혹여나 블로그 작성을 한다면 검증된 내용으로만 작성하길 바랄게요...!)
	 *  
	 * 		Exception(예외)와 Error(예외)
	 * 		- Exception : 개발 알고리즘에서 발생하는 오류로 개발자가 작성한 코드에서 발생하므로 예외를 상황에 맞춰 처리할 수 있다.
	 * 		- Error : 시스템에서 발생하는 심각한 수준의 에러로 개발자가 미리 예측하여 대응할수 없기 때문에 예외 처리에 대한 부분을 신경쓰지 않아도 된다.
	 * 		
	 * 		CheckedException과 UncheckedException
	 * 		- RuntimeException의 상속 여부에 따라서 CheckedException 과 UncheckedException으로 나뉘어진다. 
	 * 
	 ...
	 * 			- 해당 어노테이션 클래스 수준에서 선언되면 선언 클래스 및 해당 클래스의 모든 메소드에 기본값으로 적용된다.
	 * 			- RuntimeException 계열, Error 예외에 대해서는 Rollback이 가능하다.
	 * 
	 * 			1) isolation(격리 수준)
	 * 				- 각 트랜잭션이 존재할 때, 트랜잭션들 끼리 서로 고립된 수준을 나타내며 서로간에 가용된 데이터를 컨트롤 할지에 대한 부분들을 설정할 수 있다.
	 * 				- isolation 기본 값은 DEFAULT이다. 
	 * 				- 새롭게 시작된 트랜잭션에서만 적용되므로, Propagation.REWUIRED 또는 Prooagation.REQUIRES_NEW와 함께 사용되도록 독점 설계됨.
	 * 	
	 * 				1-1) 옵션
	 * 			
	 * 					** 용어
	 ...
	 * 									  Non-repeatable Read 및 Phantom Read가 발생 할 수 있습니다.
	 * 						> 하나의 트랜잭션 처리가 이루어진 변경 내용이 Commit 된 후 , 다른 트랜잭션에서 조회가 가능하다.
	 * 						> A 트랜잭션이 데이터를 변경하고 B 트랜잭션이 조회를 진행할 떄, B 트랜잭션은 Shared Lock이 걸린다.
	 ...
	 * 						> 트랜잭션 Commit이 일어난 데이터에 대해서 조회가 가능(트랜잭션 완료 시 까지 조회에 대한 Shared Lock이 걸리지 않음)
	 					- SERIALIZABLE : Dirty read, Non-repeatable Read 및 Phantom Read 가 방지됨을 나타내는 상수입니다.
	 ...
	 * 				- MADATORY : 현재 트랜잭션을 지원하고, 없으면 예외를 발생시킨다.
	 ...
	 * 					> 읽기전용에 대한 힌트를 분석할 수 없는 트랜잭션인 경우 throw를 던지지 않고 조용히 힌트를 무시합니다.
	 * 
	 * 
	 * 			4) timeout (트랜잭션 제한시간)
	 * 				- 기본값은 -1로 무제한이다.
	 * 				- timeout은 클라이언트와 서버와의 통신 중, 서버측 문제로 다음 처리를 이어나가지 못한는 'Deadlock'을 방지할 수 있는 속성이다.
	 * 				- 클라이언트와 서버간의 Restful API를 개발 시 고려해볼 속성
	 * 
	 * 			5) rollbackFor
	 * 				- 트랜잭션 롤백을 유발해야 하는 예외 유형을 나타내는 0개 이상의 예외 유형을 정의합니다.
	 * 				- 기본적으로 트랜잭션은 롤백되지만, RuntimeException계열과 Error은 롤백되지 않는다.
