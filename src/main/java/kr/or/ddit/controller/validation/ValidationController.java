...
public class ValidationController {
		 ...
		 * 
		 * 1. 입력값 검증
		 * - 스프링 MVC는 Bean Validation기능을 이용해 요청 파라미터 값이 바인딩된 도메인 클래스(또는 커멘드 클래스)의 입력값 검증을 한다.
		 * 
		 * 		# 의존 관계 정의
	...
	public String registerValidatedForm01Process(@Validated Member member, BindingResult result) {
		...
		log.info("pasword : "+ member.getPassword());
	...
	// 1) 입력값 검증 대상의 도메인 클래스 직후에 BindingResult를 정의하여 에러 정보를 확인 할 수 있다(확인은 처리 메서드에서)
	 ...
	 * 			@NotEmpty 				: 문자열이 null이 이거나 비어있는지 검사한다.
	 ...
	 * 
	 * 			그 외에도 정말 많은 어노테이션이 존재하지만, 모든 어노테이션을 다루기 어려우므로
	 * 			관심이 있는분들은 공식 문서를 참고바랍니다!
	 * 
	 * 			[테스트 시나리오]
	...
	public String registerValidation03(Model model) {
		log.info("registerValidation03() 실행중...!");
		model.addAttribute("member", new Member());
	...
	public String registerValidationResult3(@Validated Member member, BindingResult result) {
		log.info("registerValidationResult3() 실행중...!");
