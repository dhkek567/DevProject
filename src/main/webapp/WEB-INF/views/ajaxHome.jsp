...
$(function(){
	...
	putXmlBtn.on("click", function(){
		...
		$.ajax({
			...
			url : "/board/"+boardNo,
			data : xmlData,
			// 요청 ContentType 헤더 값을 "application/xml"로 지정한다.
	...
	getJsonBtn.on("click", function(){
		...
		$.ajax({
			type : "get",
			url : "/board/"+boardNo,
			// 요청 Accept 헤더 값을 "application/json" 미디어 타입으로 지정한다
			headers : {
...
function xmlToString(xmlData){
	...
	// window.ActiveXObject는 ActiveXObject를 지원하는 브라우저면  Object를 리턴하고 그렇지 않으면 null을 리턴한다.
	
	if(window.ActiveXObject){ // IE 기반 및 타 브라우저 하위 버젼
		xmlString = xmlData.xml;
	}else{
		// 파싱된 XML 데이터를 Stirng으로 변환
