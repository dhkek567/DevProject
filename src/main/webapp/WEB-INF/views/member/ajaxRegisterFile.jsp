...
$(function(){
	...
	inputFile.on("change", function(event){
		...
		if(isImageFile(file)){
			// ajax로 파일을 컨트롤 할 때는 formData를 이용한다. 
			// append() 를 이용해서 추가하고자 하는 파일 데이터를 key, value형식으로 넣는다.
			// 일반 데이터도 마찬가지로 append() 를 이용해서 추가하고자 하는 데이터를 key, value형식으로 넣는다.
			...
			// contentType :  Content-Type 을 설정 시, 사용하는 해당 설정의 기본값은  "application/x-www-form-urlencoded; charset=utf-8"이다.
			// 					하여, 기본값으로 나가지 않고 "multipart/form-data; 로 나갈 수 있도록 설정을 false합니다.
			// request 요청에서 content-Type을 확인해보면 "multipart/form-data; boundary=----WevKitFormBoundary7YDjpu8j8iHJIkkjkkUhkb..."
			...
			$.ajax({
				...
				data : formData,
				processData : false,
				// contentType 에 false로 하면 multipart/form-data로 넘어간다 
				contentType : false,
				dataType : "text",
				success : function(result){
					...
					if(result ==="UPLOAD SUCCESS"){
						...
						// base64 인토딩 설정에 의한 이미지 데이터가 출력된다. 
					}
				}
			});
		}else{
