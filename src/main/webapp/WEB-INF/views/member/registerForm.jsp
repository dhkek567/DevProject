...
<body>
	...
	<a href="/register?userId=hongkd&password=1234">Register01</a><br>
	...
	<p>4)HTML Form 필드가 여러개일 경우에 컨트롤러 매개변수의 위치가 변경되면 데이터는 어떻게 들어올까?</p>
	...
	<p>3) @RequestParam 어노테이션을 사용하여 특정한 HTML Form 필드명을 지정하여 요청을 처리한다.</p>
	...
	<p>1) 폼 텍스트 필드 소요값을 자바빈즈 매개변수로 처리한다. </p>
	...
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=1234">Register01</a>
	...
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=2018-09-08">Register02</a>
	<font color="red">400-Bad Request 에러 발생</font>
	<br>
	
	<p>3) 쿼리 파라미터(dateOfBirth=20180908)로 전달받은 값이 날짜 문자열 형식에 맞지 않아 Date 타입으로 변환에 실패한다. </p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=20180908">Register01</a>
	<font color="red">400-Bad Request 에러 발생</font>
	<br>
	
	
	<p>4) 쿼리 파라미터(dateOfBirth=2018/09/08)로 전달받은 값이 날짜 문자열 형식에 맞지 않아 Date 타입으로 변환에 실패한다. </p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=2018/09/08">Register01</a>
	<font color="green">SUCCESS</font>
	<br>
	
	<p>5) Member 매개 변수와 쿼리 파라미터(dateOfBirth=20180908)로 전달받은 값이 날짜 문자열 형식에 맞지 않아 Date 타입으로 변환에 실패한다. </p>
	<a href="/registerByGet02?userId=hongkd&dateOfBirth=20180908">Register01</a>
	<font color="red">400-Bad Request 에러 발생</font>
	<br>
	...
	<p>5) 폼 selectBox 요소값을 기본 데이터 타입인 문자열 타입 매개변수로 처리한다. </p>
	...
	<p>10) 폼 체크박스 요소 값을 기본 데이터 타입인 배열 타입 매개변수로 처리한다. </p>
	...
	<p>14) 폼 택스트 필드 요소값 자바빈즈 매개변수로 처리한다. </p>
	...
	<!-- MemberVO에 있는 Address의 변수명으로 작성  -->
	...
	<!-- MemberVO에 있는 List<Card>의 변수명으로 작성  -->
	<p>16) 폼 택스트 핑드 요소값을 중첩된 자바빈즈 매개변수로 처리한다.</p>
	...
	<p>18) 폼 택스트 영역 요소값을 Date 타입 매개 변수로 처리한다.</p>
	<form action="/registerDate01" method="post">
		dateOfBirth : <input type="text" name="dateOfBirth"><br>
		<input type="submit" value="전송">
	</form> <br>

	<p>19) 폼 택스트 영역 요소값을 @DateTimeFormat 어노테이션을 지정하여 Date 타입 매개 변수로 처리한다.</p>
	...
	<p>1) 파일 업로드 폼 파일 요소값을 스프링MVC가 지원하는 MultipartFile 매개 변수로 처리한다. </p>
	<form action="/registerFile01" method="post" enctype="multipart/form-data">
		<input type="file" name="picture"/>
		<input type="submit" value="전송">
	</form>

	<p>2) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을  MultipartFile 매개 변수와 문자열 매개변수로 처리한다. </p>
	...
	<p>3) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값을  MultipartFile 매개 변수와 자바빈즈 매개변수로 처리한다. </p>
	...
	<p>5) 여러개의 파일업로드 폼 파일 요소값을  MultipartFile 매개 변수로 처리한다. </p>
